    ...
    else
    {
        ...
        else
        {
            ...
            for (i = 0; i < (row_tail - row_head); i++)
            {
                ...
                //                    mat_result->data[i * (mat_result->column) + j] = _mat->data[(i + row_head) *
                //                    (_mat->column) +
                //                                                                                (j + column_head)];
                //                }
            }
        ...
        for (j = 0; j < mat_result->column; j++)
        {
            ...
            { /*这里的双判断，可以优化*/
                if ((j >= column_left) && (j < (column_left + _mat->column)))
                {
                    ...
                    //                    mat_result->data[i * (mat_result->column) + j] = _mat->data[(_mat->column) *
                    //                    (i - row_up) +
                    //                                                                                (j -
                    //                                                                                column_left)];
                }
...
* [注] 最左侧，和最上侧，row_up和column_left取值为_HEAD_(1)*/
    ...
    {
        // using memory-copy to speed up.
        memcpy(&(data[(row_up + i) * column + (column_left)]), &(_mat_full->data[i * full_column]),
...
* 2. [求解-特征向量参考](https://blog.csdn.net/w_weixiaotao/article/details/111868086) */
    ...
    if (_mat->column == _mat->row)
    {
        ...
        for (eigen_count = 0; eigen_count < dim; eigen_count++)
        {
            ...
            for (i = 0; i < dim; i++)
            {
                _mat_->data[i * _mat_->column + i] -=
                    eigen_value_temp; // 注意: 这里计算 (A-lamda*I), 当A为I/diag时，可能存在问题;
            }
...
* */
    Matrix *_mat_pinv = NULL, *mat_T = NULL, *mat_mat_T = NULL, *mat_T_mat = NULL, *inv_mat_T_mat = NULL,
           *inv_mat_mat_T = NULL, *mat_svd_inv_dia_T = NULL, *mat_temp_1 = NULL, *mat_temp_2 = NULL;
    ...
    switch (_method_)
    {
        ...
        case _SVD_:
            ...
            for (i = 0; i < _mat->row; i++)
            {
                ...
                {
                    mat_svd_inv_dia_T->data[i * (mat_svd_inv_dia_T->column) + i] =
                        1 / (val_temp + _APPROXIMATELY_ZERO_);
                }
            }
