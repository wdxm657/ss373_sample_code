...
static char protocol_data = 8;
static char protocol_pority = 'n';
static char protocol_stop = 1;
...
static void print_usage(const char *prog)
{
    ...
    printf("UART demo for read/write test.\n\n");
    printf("Options:\n");
    printf("  -D, --device=PATH    TTY device (default: %s)\n", DEV_NAME);
    printf("  -b, --baudrate=RATE  Baud rate (e.g. 115200)\n");
    printf("  -d, --data=BITS      Data bits (5/6/7/8)\n");
    printf("  -p, --parity=MODE    Parity: n/N, o/O, e/E, s/S (default: n)\n");
    printf("  -s, --stop=BITS      Stop bits (1/2)\n");
    printf("  -c, --CTSRTS=0|1     Hardware flow control (0=off, 1=on)\n");
    printf("  -h, --help           Show this help and exit\n");
    printf("\nExample:\n");
    printf("  %s -D /dev/ttyS1 -b 115200 -d 8 -p n -s 1 -c 0\n", prog);
}
...
static int parse_opts(int argc, char *argv[])
{
    while (1)
    {
        ...
        c = getopt_long(argc, argv, "D:b:d:p:s:c:h",
                        lopts, NULL);
...
void *ST_WriteProcess(void *args)
{
    ...
    while (1)
    {
        ...
        memcpy(szBuf, p_buf, s_len);

        len = write(*fd, szBuf, s_len);
        do
        {
            ioctl(*fd, TIOCSERGETLSR, &lsr);
        } while (!(lsr & TIOCSER_TEMT));
