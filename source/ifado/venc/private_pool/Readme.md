...
## 一、demo场景
    该场景下各模块使用的是各自的私有池private pool，而不是使用mma中的公共池。
    vif->isp->scl->main venc->rtsp0

    参数说明： vif接sensor pad0，isp/scl/main venc使用dev0 chn0

    绑定模式：vif->isp->scl->venc均为framemode，isp使用的是chn private pool，其余模块使用的是dev private pool
...
2. 进到sample_code目录执行`make clean && make source/ifado/venc/private_pool`进行编译;

3. 到`sample_code/out/arm/app/prog_venc_private_pool`获取可执行文件;
...
1.   按`./prog_venc_private_pool`运行场景private pool；
> 按`./prog_venc_private_pool iqbin xxx`运行，可在场景的基础上加载指定iqbin文件，xxx为iqbin文件路径
