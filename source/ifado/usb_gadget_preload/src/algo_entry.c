/* SigmaStar trade secret */
/* Copyright (c) [2019~2020] SigmaStar Technology.
All rights reserved.

Unless otherwise stipulated in writing, any and all information contained
herein regardless in any format shall remain the sole proprietary of
SigmaStar and be kept in strict confidence
(SigmaStar Confidential Information) by the recipient.
Any unauthorized act including without limitation unauthorized disclosure,
copying, use, reproduction, sale, distribution, modification, disassembling,
reverse engineering and compiling of the contents of SigmaStar Confidential
Information is unlawful and strictly prohibited. SigmaStar hereby reserves the
rights to any and all damages, losses, costs and expenses resulting therefrom.
*/

#include <isp_cus3a_if.h>
//#include <cus3a_api_wrapper.h>

int CUS3A_EnableUserspaceAE(u32 nCh, u8 bEn);
int CUS3A_EnableUserspaceAWB(u32 nCh, u8 bEn);

#define log_info 0

static int ae_init(void* pdata, ISP_AE_INIT_PARAM *init_state)
{
    CamOsPrintf("**[demo] ae_init ,shutter=%d,shutter_step=%d,sensor_gain_min=%d,sensor_gain_max=%d **\n",
            init_state->shutter,
            init_state->shutter_step,
            init_state->sensor_gain,
            init_state->sensor_gain_max
          );
    return 0;
}

static void ae_release(void* pdata)
{
    CamOsPrintf("**[demo] ae_release **\n");
}

static void ae_run(void* pdata, const ISP_AE_INFO *info, ISP_AE_RESULT *result)
{
    unsigned int max = info->AvgBlkY*info->AvgBlkX;
    unsigned int avg=0;
    unsigned int n;

    result->Change              = 0;
    result->i4BVx16384          = 16384;
    result->HdrRatio            = 10;
    result->IspGain             = 1024;
    result->SensorGain          = 4096;
    result->Shutter             = 20000;
    result->IspGainHdrShort     = 1024;
    result->SensorGainHdrShort  = 1024;
    result->ShutterHdrShort     = 1000;

    for(n=0;n<max;++n)
    {
        avg += info->avgs[n].y;
    }
    avg /= max;

    result->AvgY         = avg;

    unsigned int y_lower = 0x48;
    unsigned int y_upper = 0x58;
    unsigned int change_ratio = 6; // percentage
    unsigned int Gain_Min = 1024*2;
    unsigned int Gain_Max = 1024*1000;
    unsigned int Shutter_Min = 150;
    unsigned int Shutter_Max = 33333;

    result->SensorGain = info->SensorGain;
    result->Shutter = info->Shutter;

    if(avg<y_lower){
        if (info->Shutter<Shutter_Max){
            result->Shutter = info->Shutter + (info->Shutter*change_ratio/100);
            if (result->Shutter > Shutter_Max) result->Shutter = Shutter_Max;
        }else{
            result->SensorGain = info->SensorGain + (info->SensorGain*change_ratio/100);
            if (result->SensorGain > Gain_Max) result->SensorGain = Gain_Max;
        }
        result->Change = 1;
    }else if(avg>y_upper){
        if (info->SensorGain>Gain_Min){
            result->SensorGain = info->SensorGain - (info->SensorGain*change_ratio/100);
            if (result->SensorGain < Gain_Min) result->SensorGain = Gain_Min;
        }else{
            result->Shutter = info->Shutter - (info->Shutter*change_ratio/100);
            if (result->Shutter < Shutter_Min) result->Shutter = Shutter_Min;
        }
        result->Change = 1;
    }
    #if log_info
    CamOsPrintf("Image avg = 0x%X \n", avg);
    CamOsPrintf("SensorGain: %d -> %d \n", info->SensorGain, result->SensorGain);
    CamOsPrintf("Shutter: %d -> %d \n", info->Shutter, result->Shutter);
    #endif
}

static int awb_init(void *pdata, ISP_AWB_INIT_PARAM *param)
{
    CamOsPrintf("**[demo] awb_init **\n");
    return 0;
}

static void awb_run(void* pdata, const ISP_AWB_INFO *info, ISP_AWB_RESULT *result)
{
    static u32 count = 0;
    int avg_r = 0;
    int avg_g = 0;
    int avg_b = 0;
    int tar_rgain = 1024;
    int tar_bgain = 1024;
    int x = 0;
    int y = 0;

    result->R_gain = info->CurRGain;
    result->G_gain = info->CurGGain;
    result->B_gain = info->CurBGain;
    result->Change = 0;
    result->ColorTmp = 6000;

    if (++count % 4 == 0)
    {
        //center area YR/G/B avg
        for (y = 30; y<60; ++y)
        {
            for (x = 32; x<96; ++x)
            {
                avg_r += info->avgs[info->AvgBlkX*y + x].r;
                avg_g += info->avgs[info->AvgBlkX*y + x].g;
                avg_b += info->avgs[info->AvgBlkX*y + x].b;
            }
        }
        avg_r /= 30 * 64;
        avg_g /= 30 * 64;
        avg_b /= 30 * 64;

        if (avg_r <1)
            avg_r = 1;
        if (avg_g <1)
            avg_g = 1;
        if (avg_b <1)
            avg_b = 1;

#if log_info
        CamOsPrintf("AVG R / G / B = %d, %d, %d \n", avg_r, avg_g, avg_b);
#endif

        // calculate Rgain, Bgain
        tar_rgain = avg_g * 1024 / avg_r;
        tar_bgain = avg_g * 1024 / avg_b;

        if (tar_rgain > info->CurRGain) {
            if (tar_rgain - info->CurRGain < 384)
                result->R_gain = tar_rgain;
            else
                result->R_gain = info->CurRGain + (tar_rgain - info->CurRGain)/10;
        }else{
            if (info->CurRGain - tar_rgain < 384)
                result->R_gain = tar_rgain;
            else
                result->R_gain = info->CurRGain - (info->CurRGain - tar_rgain)/10;
        }

        if (tar_bgain > info->CurBGain) {
            if (tar_bgain - info->CurBGain < 384)
                result->B_gain = tar_bgain;
            else
                result->B_gain = info->CurBGain + (tar_bgain - info->CurBGain)/10;
        }else{
            if (info->CurBGain - tar_bgain < 384)
                result->B_gain = tar_bgain;
            else
                result->B_gain = info->CurBGain - (info->CurBGain - tar_bgain)/10;
        }

        result->Change = 1;
        result->G_gain = 1024;

#if log_info
        CamOsPrintf("[awb input] r=%d, g=%d, b=%d \n", info->CurRGain, info->CurGGain, info->CurBGain);
        CamOsPrintf("[awb output] r=%d, g=%d, b=%d \n", result->R_gain, result->G_gain, result->B_gain);
#endif
    }
}

static void awb_release(void *pdata)
{
    CamOsPrintf("**[demo] awb_release **\n");
}

static int af_init(void *pdata, ISP_AF_INIT_PARAM *param)
{
#if 0
    MI_U32 u32ch = 0;
    MI_U8 u8win_idx = 16;
    ISPMID_AF_ROI_MODE taf_roimode;

    CamOsPrintf("**[demo] af_init **\n");

#if 1
    //Init Normal mode setting
    taf_roimode.mode = AF_ROI_MODE_NORMAL;
    taf_roimode.u32_vertical_block_number = 1;
    ISPMID_SetAFRoiMode(u32ch, &taf_roimode);

    static CusAFWin_t afwin[16] =
    {
        // x_start need to start from 16

        //{ 0, {   16,    0,  1023,  1023}},  //for full image
        { 0, {  16,    0,  255,  255}},
        { 1, { 256,    0,  511,  255}},
        { 2, { 512,    0,  767,  255}},
        { 3, { 768,    0, 1023,  255}},
        { 4, {  16,  256,  255,  511}},
        { 5, { 256,  256,  511,  511}},
        { 6, { 512,  256,  767,  511}},
        { 7, { 768,  256, 1023,  511}},
        { 8, {  16,  512,  255,  767}},
        { 9, { 256,  512,  511,  767}},
        {10, { 512,  512,  767,  767}},
        {11, { 768,  512, 1023,  767}},
        {12, {  16,  768,  255, 1023}},
        {13, { 256,  768,  511, 1023}},
        {14, { 512,  768,  767, 1023}},
        {15, { 768,  768, 1023, 1023}}
    };
    for(u8win_idx = 0; u8win_idx < 16; ++u8win_idx)
    {
        MI_ISP_CUS3A_SetAFWindow(u32ch, &afwin[u8win_idx]);
    }
#endif

#if 1 //TARGET_CHIP_I6E
    //set AF Filter
    static CusAFFilter_t affilter =
    {
        //filter setting with sign value
        //convert to hw format (sign bit with msb)
        37, 0, 37+512, 107+128, 49, 0, 1023, 0, 1023,   //low
        19, 0, 19+512, 122+128, 59, 0, 1023, 0, 1023,   //high
        1, 37, 0, 37+512,      25, 28, 1, 32, 0, 32+512,  41+128, 0,
        1, 19, 0, 19+512,  73+128, 36, 1, 17, 0, 17+512,  91+128, 30,
    };
#endif

    MI_ISP_CUS3A_SetAFFilter(0, &affilter);

    //set AF Sq
    CusAFFilterSq_t sq = {
        .bSobelYSatEn = 0,
        .u16SobelYThd = 1023,
        .bIIRSquareAccEn = 1,
        .bSobelSquareAccEn = 0,
        .u16IIR1Thd = 0,
        .u16IIR2Thd = 0,
        .u16SobelHThd = 0,
        .u16SobelVThd = 0,
        .u8AFTblX = {6,7,7,6,6,6,7,6,6,7,6,6,},
        .u16AFTblY = {0,32,288,800,1152,1568,2048,3200,3872,4607,6271,7199,8191},
    };


    MI_ISP_CUS3A_SetAFFilterSq(0, &sq);
    CamOsPrintf("**[%s] af_init done **\n", __FUNCTION__);
#else
    CamOsPrintf("**[demo] af_init **\n");
#endif
    return 0;
}

static void af_release(void *pdata)
{
    CamOsPrintf("**[%s] af_release **\n", __FUNCTION__);
}

static void af_run(void *pdata, const ISP_AF_INFO *af_info, ISP_AF_RESULT *result)
{
#if log_info
    int i = 0, x = 0;
    x = 0;
    //for (i = 0; i < 16; i++)
    for (i = 0; i < 2; i++)
    {
        CamOsPrintf("\n[AF]win%d-%d iir0:0x%02x%02x%02x%02x%02x, iir1:0x%02x%02x%02x%02x%02x, luma:0x%02x%02x%02x%02x, sobelh:0x%02x%02x%02x%02x%02x, sobelv:0x%02x%02x%02x%02x%02x ysat:0x%02x%02x%02x",
               x, i,
               af_info->pStats->stParaAPI[x].high_iir[4 + i * 5], af_info->pStats->stParaAPI[x].high_iir[3 + i * 5], af_info->pStats->stParaAPI[x].high_iir[2 + i * 5], af_info->pStats->stParaAPI[x].high_iir[1 + i * 5], af_info->pStats->stParaAPI[x].high_iir[0 + i * 5],
               af_info->pStats->stParaAPI[x].low_iir[4 + i * 5], af_info->pStats->stParaAPI[x].low_iir[3 + i * 5], af_info->pStats->stParaAPI[x].low_iir[2 + i * 5], af_info->pStats->stParaAPI[x].low_iir[1 + i * 5], af_info->pStats->stParaAPI[x].low_iir[0 + i * 5],
               af_info->pStats->stParaAPI[x].luma[3 + i * 4], af_info->pStats->stParaAPI[x].luma[2 + i * 4], af_info->pStats->stParaAPI[x].luma[1 + i * 4], af_info->pStats->stParaAPI[x].luma[0 + i * 4],
               af_info->pStats->stParaAPI[x].sobel_h[4 + i * 5], af_info->pStats->stParaAPI[x].sobel_h[3 + i * 5], af_info->pStats->stParaAPI[x].sobel_h[2 + i * 5], af_info->pStats->stParaAPI[x].sobel_h[1 + i * 5], af_info->pStats->stParaAPI[x].sobel_h[0 + i * 5],
               af_info->pStats->stParaAPI[x].sobel_v[4 + i * 5], af_info->pStats->stParaAPI[x].sobel_v[3 + i * 5], af_info->pStats->stParaAPI[x].sobel_v[2 + i * 5], af_info->pStats->stParaAPI[x].sobel_v[1 + i * 5], af_info->pStats->stParaAPI[x].sobel_v[0 + i * 5],
               af_info->pStats->stParaAPI[x].ysat[2 + i * 3], af_info->pStats->stParaAPI[x].ysat[1 + i * 3], af_info->pStats->stParaAPI[x].ysat[0 + i * 3]
              );
    }
#endif
}

void DrvAlgoEntry(void)
{
    ISP_AE_INTERFACE tAeIf;
    ISP_AWB_INTERFACE tAwbIf;
    ISP_AF_INTERFACE tAfIf;

    /*CUS3A global init*/
    CUS3A_Init(E_ISP_DEV_0);

    /*CUS3A ISP0 cteate channel*/
    CUS3A_CreateChannel(E_ISP_DEV_0, E_ISP_CH_0);
    //CUS3A_CreateChannel(E_ISP_DEV_0, E_ISP_CH_1);
    //CUS3A_CreateChannel(E_ISP_DEV_0, E_ISP_CH_2);
    //CUS3A_CreateChannel(E_ISP_DEV_0, E_ISP_CH_3);

    /* Register customer AE algorithm on ISP0, Adaptor1*/
    tAeIf.init = ae_init;
    tAeIf.run = ae_run;
    tAeIf.release = ae_release;
    tAeIf.ctrl = NULL;
    tAeIf.pdata = NULL;
    CUS3A_RegInterfaceEX(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AE, &tAeIf);

    /* Register customer AWB algorithm on ISP0, Adaptor1*/
    tAwbIf.init = awb_init;
    tAwbIf.run = awb_run;
    tAwbIf.release = awb_release;
    tAwbIf.ctrl = NULL;
    tAwbIf.pdata = NULL;
    CUS3A_RegInterfaceEX(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AWB, &tAwbIf);

    /* Register customer AF algorithm on ISP0, Adaptor1*/
    tAfIf.init = af_init;
    tAfIf.run = af_run;
    tAfIf.release = af_release;
    tAfIf.ctrl = NULL;
    tAfIf.pdata = NULL;
    CUS3A_RegInterfaceEX(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AF, &tAfIf);

    /* Select AE algorithm for ISP0*/
    CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_NATIVE, E_ALGO_TYPE_AE);
    //CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AE);

    /* Select AWB algorithm for ISP0 */
    CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_NATIVE, E_ALGO_TYPE_AWB);
    //CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AWB);

    /* Select AF algorithm for ISP0 */
    CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_0, E_ALGO_ADAPTOR_NATIVE, E_ALGO_TYPE_AF);
#if 0
    /* Register customer AE algorithm on ISP0, Adaptor1*/
    tAeIf.init = ae_init;
    tAeIf.run = ae_run;
    tAeIf.release = ae_release;
    tAeIf.ctrl = NULL;
    tAeIf.pdata = NULL;
    CUS3A_RegInterfaceEX(E_ISP_DEV_0, E_ISP_CH_1, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AE, &tAeIf);

    /* Register customer AWB algorithm on ISP0, Adaptor1*/
    tAwbIf.init = awb_init;
    tAwbIf.run = awb_run;
    tAwbIf.release = awb_release;
    tAwbIf.ctrl = NULL;
    tAwbIf.pdata = NULL;
    CUS3A_RegInterfaceEX(E_ISP_DEV_0, E_ISP_CH_1, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AWB, &tAwbIf);

    /* Select AE algorithm for ISP0*/
    CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_1, E_ALGO_ADAPTOR_NATIVE, E_ALGO_TYPE_AE);
    //CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_1, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AE);

    /* Select AWB algorithm for ISP0 */
    CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_1, E_ALGO_ADAPTOR_NATIVE, E_ALGO_TYPE_AWB);
    //CUS3A_SetAlgoAdaptor(E_ISP_DEV_0, E_ISP_CH_1, E_ALGO_ADAPTOR_1, E_ALGO_TYPE_AWB);
#endif
}
