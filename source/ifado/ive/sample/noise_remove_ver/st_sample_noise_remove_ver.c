...
#define OUTPUT_NAME  "./out/ive/Output_NoiseRemoveVer_640x480_U8C1.bin"
...
int main()
{
    ...
    MVE_IVE_NoiseRemoveVerCtrl_t ctrl =
    {
        .u8VerThr       = 24,   //Range : 1 ~ 100
        .u8VerThrMax    = 79    //Range : 1 ~ 100
    };
    ...
    ret = MI_IVE_NoiseRemoveVer(handle, &src, &ctrl, 0);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_NoiseRemoveVer() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(src);
        ST_Common_IveDestroy(handle);
        return ret;
    }
