...
#define INPUT_NAME_0    "./resource/input/ive/640x360_YUV420SP_sori.bin"
#define INPUT_NAME_1    "./resource/input/ive/640x360_YUV420SP_bg.bin"
#define INPUT_NAME_2    "./resource/input/ive/640x360_U8C1_ymask.bin"
#define OUTPUT_PATH     "./out/ive/"
#define OUTPUT_NAME     "./out/ive/Output_AlphaBlending_640x360_U8C1.bin"
...
int main()
{
    ...
    MI_IVE_SrcImage_t src0, src1, alpha;
    ...
    ret = ST_Common_IveAllocateImage(&alpha, E_MI_IVE_IMAGE_TYPE_U8C1, RAW_WIDTH, RAW_WIDTH, RAW_HEIGHT);
    if (ret != MI_SUCCESS)
    {
        printf("Can't allocate input alpha buffer\n");
    ...
    ret = ST_Common_IveInitInputImage(&alpha, INPUT_NAME_2);
    if (ret != MI_SUCCESS)
    {
    ...
    ret = MI_IVE_AlphaBlending(handle, &src0, &src1, &alpha, &dst, 1);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_AlphaBlending() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(dst);
        CHECK_AND_FREE_IMAGE(alpha);
        CHECK_AND_FREE_IMAGE(src1);
        CHECK_AND_FREE_IMAGE(src0);
