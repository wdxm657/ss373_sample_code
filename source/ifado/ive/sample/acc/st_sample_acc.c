...
#define INPUT_NAME_0    "./resource/input/ive/1280x720_U8C1_square.bin"
#define INPUT_NAME_1    "./resource/input/ive/1280x720_U8C1_round.bin"
#define OUTPUT_PATH     "./out/ive/"
#define OUTPUT_NAME     "./out/ive/Output_Acc_1280x720_U8C1.bin"
...
int main()
{
    ...
    MVE_IVE_AccCtrl_t ctrl =
    {
        .enMode = E_MI_IVE_ACC_MODE_INCREASE
    };
    ...
    ret = MI_IVE_Acc(handle, &src0, &src1, &dst, &ctrl, 0);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_Acc() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(dst);
        CHECK_AND_FREE_IMAGE(src1);
        CHECK_AND_FREE_IMAGE(src0);
        ST_Common_IveDestroy(handle);
