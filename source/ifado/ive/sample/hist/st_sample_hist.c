...
#define OUTPUT_NAME  "./out/ive/Output_Hist_U32x256.bin"
...
int main()
{
    ...
    ret = ST_Common_IveAllocateBuffer(&hist, sizeof(MI_U32)*256);
    if (ret != MI_SUCCESS)
    {
    ...
    ret = ST_Common_IveSaveOutputBuffer(&hist, OUTPUT_NAME, sizeof(MI_U32)*256);
    if (ret != MI_SUCCESS)
    {
