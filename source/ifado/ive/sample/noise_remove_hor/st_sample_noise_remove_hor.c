...
#define OUTPUT_NAME  "./out/ive/Output_NoiseRemoveHor_640x480_U8C1.bin"
...
int main()
{
    ...
    MVE_IVE_NoiseRemoveHorCtrl_t ctrl =
    {
        .u8HorThr       = 25,    // Range : 1 ~ 100
        .u8HorThrMax    = 79     // Range : 1 ~ 100
    };
    ...
    ret = MI_IVE_NoiseRemoveHor(handle, &src, &ctrl, 0);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_NoiseRemoveHor() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(src);
        ST_Common_IveDestroy(handle);
        return ret;
    }
