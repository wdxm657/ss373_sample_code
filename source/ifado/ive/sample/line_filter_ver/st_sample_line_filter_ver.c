...
#define OUTPUT_NAME  "./out/ive/Output_LineFilterVer_640x480_U8C1.bin"
...
int main()
{
    ...
    MVE_IVE_LineFilterVerCtrl_t ctrl =
    {
        .u8VerThr = 25   //Gap min size (range : 1 ~ 64)
    };
    ...
    ret = MI_IVE_LineFilterVer(handle, &src, &ctrl, 0);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_LineFilterVer() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(src);
        ST_Common_IveDestroy(handle);
        return ret;
    }
