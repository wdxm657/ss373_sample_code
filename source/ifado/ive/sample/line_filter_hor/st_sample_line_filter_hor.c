...
#define OUTPUT_NAME  "./out/ive/Output_LineFilterHor_640x480_U8C1.bin"
...
int main()
{
    ...
    MVE_IVE_LineFilterHorCtrl_t ctrl =
    {
        .u8GapMinLen    = 10,    //Gap min size (range : 1 ~ 20)
        .u8DensityThr   = 20,    //Density threshold (range : 1 ~ 50)
        .u8HorThr       = 20     //Horizontal size threshold (range : 1 ~ 50)
    };
    ...
    ret = MI_IVE_LineFilterHor(handle, &src, &ctrl, 0);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_LineFilterHor() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(src);
        ST_Common_IveDestroy(handle);\
        return ret;
    }
    ...
    ret = ST_Common_IveSaveOutputImage(&src, OUTPUT_NAME);
    if (ret != MI_SUCCESS)
    {
