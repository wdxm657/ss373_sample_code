...
#define OUTPUT_NAME  "./out/ive/Output_Adpthresh_640x480_U8C1.bin"
...
int main()
{
    ...
    MI_IVE_SrcImage_t src , integ_src;
    MI_IVE_DstImage_t dst;
    MI_IVE_IntegCtrl_t integ_ctrl =
        {
            .eOutCtrl = E_MI_IVE_INTEG_OUT_CTRL_SUM
        };

    MVE_IVE_AdpThreshCtrl_t adp_thresh_ctrl =
        {
    ...
    ret = ST_Common_IveAllocateImage(&integ_src, E_MI_IVE_IMAGE_TYPE_U32C1, RAW_WIDTH, RAW_WIDTH, RAW_HEIGHT);
    if (ret != MI_SUCCESS)
    {
    ...
    if (ret != MI_SUCCESS)
    {
        ...
        CHECK_AND_FREE_IMAGE(integ_src);
        CHECK_AND_FREE_IMAGE(src);
        ST_Common_IveDestroy(handle);
        return ret;
    }
    ...
    ret = MI_IVE_Integ(handle, &src, &integ_src, &integ_ctrl, 0);
    if (ret != MI_SUCCESS)
    {
    ...
    ret = MI_IVE_AdpThresh(handle, &src, &integ_src, &dst, &adp_thresh_ctrl, 0);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_AdpThresh() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(dst);
        CHECK_AND_FREE_IMAGE(integ_src);
        CHECK_AND_FREE_IMAGE(src);
        ST_Common_IveDestroy(handle);
