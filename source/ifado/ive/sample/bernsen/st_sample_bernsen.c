...
#define INPUT_NAME   "./resource/input/ive/1280x720_U8C1_text.bin"
#define OUTPUT_PATH  "./out/ive/"
#define OUTPUT_NAME  "./out/ive/Output_Bernsen_1280x720_U8C1.bin"
...
int main()
{
    ...
    MVE_IVE_BernsenCtrl_t ctrl =
    {
        .enMode         = E_MI_IVE_BERNSEN_MODE_NORMAL,
        .u8WinSize      = 3,
        .u8Thr          = 0
    };
    ...
    ret = MI_IVE_Bernsen(handle, &src, &dst, &ctrl, 1);
    if (ret != MI_SUCCESS)
    {
        printf("MI_IVE_Bernsen() return ERROR 0x%X\n", ret);
        CHECK_AND_FREE_IMAGE(dst);
        CHECK_AND_FREE_IMAGE(src);
        ST_Common_IveDestroy(handle);
        return ret;
