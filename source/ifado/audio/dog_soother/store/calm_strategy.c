...
int calm_strategy_load(void)
{
    FILE *fp;
    ds_strategy_file_t f;

    fp = fopen(DS_STRATEGY_PATH, "rb");
    if (!fp)
    {
        return -1;
    }
    ...
    g_st.mode = (f.mode <= DS_CALM_MODE_MANUAL) ? (ds_calm_mode_t)f.mode : DS_CALM_MODE_AUTO;
...
uint16_t calm_strategy_fill_get_rsp(uint8_t *rsp, uint16_t rsp_cap)
{
    uint16_t n;
    uint8_t i;
